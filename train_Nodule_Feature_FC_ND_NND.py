...
DATA_FOLDER = '/home/kshitij/Desktop/Dataset/Features/KAGGLE_NODULE_CROP_ALL_NPY_VAL3/'
CSV_KAGGLE_PATH = '/home/kshitij/Desktop/Dataset/stage1_labels.csv'
...
if __name__ == '__main__':
    ...
    NonNodule_traing_filelist, NonNodule_test_filelist = Kaggle_Data_Load(DATA_FOLDER, CSV_KAGGLE_PATH, Training_Sample = 2000, Test_Sample = 50)
    ...
    # if os.path.isfile('./Nodule_Nodule_Feature_FC_ND_NND.hdf5'):
    #    print ("load ./Nodule_Nodule_Feature_FC_ND_NND.hdf5")
    #    nodule_model.model = load_model('./Nodule_Nodule_Feature_FC_ND_NND.hdf5')
    callbacks = [
        EarlyStopping(monitor='val_loss', patience=10000, verbose=1),
        history,
        ModelCheckpoint('Nodule_Nodule_Feature_FC_ND_NND2.hdf5', monitor='val_loss', save_best_only=True, verbose=1),
        ]
    batch_size = 2000
    epoch = 15
    ...
    # history = nodule_model.model.fit_generator(generator=generate_batch(Train_feature_label, batch_size),
    #                       nb_epoch=epoch,
    #                       samples_per_epoch=samples_per_epoch,
    #                       validation_data=generate_batch(Test_feature_label, batch_size),
    #                       nb_val_samples=nb_val_samples,use_multiprocessing=True,
    #                       verbose=1,
    #                       callbacks=callbacks)
    a ,b = generate_batch(Train_feature_label, batch_size)
    c ,d = generate_batch(Test_feature_label, batch_size)
    print b
    print a
    history = nodule_model.model.fit(a,b,
                          epochs=epoch,validation_data=(c,d),
                          verbose = 1,
                          callbacks=callbacks)
    print (history.history)
    history = nodule_model.model.predict(c)
    print history
    
    nodule_model.model.save('my_modellerstar.h5')
